...
Last activity: 2026-04-27 — Quick task 260427-lyr 완료 (프로덕션 소셜 로그인 실패 핫픽스, commit 39ae868)
...
| 260427-lyr | 프로덕션 소셜 로그인(Google/Kakao/Naver) 100% 실패 핫픽스: `apps/web/app/auth/callback/page.tsx` useEffect가 Next.js 16 + React 19 환경에서 다회 발사되어 `/auth/refresh`가 동시 두 번 호출 → `auth.service.ts:167-174` 토큰 도난 탐지가 정상 동작하여 패밀리 전체 revoke → 401 + 사용자 측 race로 "로그인 실패" 토스트. `useRef` guard로 마운트당 1회 실행 보장 (+9/-1). gcloud 로그상 Chrome/Safari 모두 `200 /auth/refresh` 직후 5ms 안에 `401 /auth/refresh` 패턴 100% 재현. 백엔드 보안 정책은 그대로 유지. | 2026-04-27 | 39ae868 | [260427-lyr-social-login-refresh-double-fire-fix](./quick/260427-lyr-social-login-refresh-double-fire-fix/) |
