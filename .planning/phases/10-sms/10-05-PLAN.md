...
must_haves:
  truths:
    ...
    - "SmsModule → BookingModule import가 순환 의존을 만들지 않음이 사전 검증됨"
  artifacts:
...
<task type="auto" tdd="true">
  <id>10-05-T1</id>
  <name>Task 1: SmsModule에 BookingModule import 추가 (순환 의존 사전 검증 포함)</name>
  ...
  <read_first>
    ...
    - apps/api/src/modules/auth/auth.module.ts (AuthModule imports 확인 — SmsModule 참조 여부)
    - .planning/phases/10-sms/10-RESEARCH.md §"Pattern 1"
  </read_first>
  <action>
    **Step 0: 순환 의존 사전 검증 (Checker Blocker #2 대응)**

    BookingModule → SmsModule 역참조가 없는지 확인:
    1. `grep -rn "SmsModule\|SmsService" apps/api/src/modules/booking/` — BookingModule이 SmsModule/SmsService를 import하지 않음을 확인
    2. `grep -rn "SmsModule" apps/api/src/modules/auth/auth.module.ts` — AuthModule이 SmsModule을 imports에 포함하는 경우, AuthModule → SmsModule → BookingModule 경로 확인
    3. `grep -rn "BookingModule" apps/api/src/modules/auth/auth.module.ts` — AuthModule이 BookingModule도 import하는 경우 간접 순환 가능성 확인
    4. `grep -rn "AuthModule" apps/api/src/modules/booking/booking.module.ts` — BookingModule이 AuthModule을 import하는지 확인

    **순환 의존 발견 시 대응:**
    - BookingModule → SmsModule 직접 참조 발견: forwardRef() 사용 또는 REDIS_CLIENT를 별도 shared provider로 분리
    - 간접 순환(BookingModule → AuthModule → SmsModule → BookingModule): REDIS_CLIENT를 app.module.ts 레벨 global provider로 올리거나, 순환 체인을 끊는 forwardRef() 적용

    **순환 없음 확인 후 진행:**

    **Step 1: `sms.module.ts`를 수정하여 `BookingModule`을 imports에 추가:**
    ...
    **Step 2: 빌드 검증:**
    `pnpm --filter @grapit/api typecheck` 실행하여 순환 의존으로 인한 컴파일 에러 없음 확인.
  </action>
  <acceptance_criteria>
    ...
    - `grep -rqL "SmsModule\|SmsService" apps/api/src/modules/booking/` 또는 해당 grep 결과 0건 (순환 없음)
    - `pnpm --filter @grapit/api typecheck` exits 0
  </acceptance_criteria>
  <verify>
    <automated>grep -q "BookingModule" apps/api/src/modules/sms/sms.module.ts && pnpm --filter @grapit/api typecheck 2>&1 | tail -5</automated>
  </verify>
  <done>SmsModule이 BookingModule import로 REDIS_CLIENT 주입 가능, 순환 의존 사전 검증 완료</done>
</task>
...
| T-10-23 | Tampering | SmsModule → BookingModule 순환 의존 | mitigate | Task 1 Step 0 사전 검증: grep으로 역참조 확인 + typecheck 빌드 검증 |
</threat_model>
