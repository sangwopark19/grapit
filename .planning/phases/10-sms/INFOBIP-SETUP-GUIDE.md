...
## 1. Infobip 포털 계정 및 SMS API v3 설정

> Grapit는 Infobip SMS API v3 (`POST /sms/3/messages`)를 사용한다. 이 절차는 **포털에서 계정을 만들고 → Sender ID를 확정하고 → API Key를 발급받아 → `.env` 3개 변수를 채우는** 흐름이다. 1인 개발자가 처음 Infobip을 만진다는 가정으로 단계별로 정리한다.
>
> *과거 Phase 10에서는 2FA PIN API (`POST /2fa/2/pin`)를 사용했으나, Phase 10.1에서 `/sms/3/messages` v3로 전환했다. Application/Message Template 사전 생성은 더 이상 필요하지 않다.*
...
Grapit의 `infobip-client.ts`는 `${baseUrl}/sms/3/messages` 으로 URL을 조립한다. 즉 **스킴(`https://`) 포함 + 끝 슬래시 없음**으로 넣어야 한다.
...
> `https://` 접두사를 빼먹으면 fetch가 상대 경로로 해석해 즉시 실패한다. 끝에 `/`를 붙이면 `//sms/3/messages`가 되어 일부 게이트웨이가 404를 돌려준다.

---

### 1.3 발신자(Sender ID) 확정

> *과거 Phase 10에서는 이 섹션이 "2FA Application 생성"이었다. `/sms/3/messages` v3 API는 Application 개념이 없으므로 Phase 10.1에서 Sender ID 확정 절차로 대체했다.*

`/sms/3/messages`는 요청 시 `from` 필드에 직접 sender를 넣으므로, **한 번 포털에서 sender를 확정해두면 끝**이다.

#### 1.3.1 Free Trial

Infobip Free Trial 계정은 공유 sender(`ServiceSMS` 등)를 강제 사용한다. `from`에 무엇을 넣든 Infobip이 공유 sender로 치환하여 발송한다. 따라서 `INFOBIP_SENDER`는 공백 또는 임의값을 넣어도 무방하다.

#### 1.3.2 프로덕션 (한국)

KISA 사전등록(2)에서 승인된 **숫자형 발신번호**를 그대로 `INFOBIP_SENDER`에 넣는다. 영문 sender ID(예: `Grapit`)는 한국 통신사(SKT/KT/LG U+)에서 치환 또는 차단되므로 KR 대상은 반드시 숫자형이어야 한다.

#### 1.3.3 값 저장 위치

`.env`의 `INFOBIP_SENDER` 환경변수에 저장한다. 이 값은 `SmsService` 생성자에서 읽어 `InfobipClient`에 전달되며, 모든 SMS 발송의 `from` 필드로 사용된다.

```bash
# .env (모노레포 루트)
INFOBIP_SENDER=0212345678    # KISA 등록 완료된 숫자형 번호 (프로덕션)
# 또는
INFOBIP_SENDER=              # Free Trial (비워두면 Infobip 기본값 사용)
```
...
> *과거 Phase 10에서는 이 섹션이 "Message Template 생성"이었다. `/sms/3/messages` v3 API는 Template 개념이 없으므로 Phase 10.1에서 본문 규격 설명으로 대체했다.*

`/sms/3/messages`는 요청 body의 `messages[].text`에 본문을 직접 넣는다. Infobip 포털에서 Template을 사전 생성할 필요가 없다.

#### 1.4.1 Grapit 고정 본문

```
[Grapit] 인증번호 <OTP> (3분 이내 입력)
```

- OTP는 `crypto.randomInt(100000, 1000000)` 으로 생성한 6자리 숫자
- 본문에 `[Grapit]`을 넣는 이유: 한국 통신사가 자동으로 `[국제발신]` 태그를 앞에 붙이므로 사용자가 발신자를 식별할 단서가 필요하다

#### 1.4.2 본문 길이 확인

- EUC-KR 1 SMS = 140 bytes (한글 약 45자)
- Grapit 본문: 한글 14자 + 숫자 6자 + 기호 = **약 50 bytes** -> 1 SMS 안에 수용
- 한국 통신사 자동 태그 `[국제발신]`(약 16 bytes)이 앞에 붙어도 1 SMS 안에 들어감

#### 1.4.3 i18n (국제 번호)

- +66, +1 등 비한국 번호용 영문 본문은 Phase 10.2 이후 검토 (deferred)
- 현재는 모든 번호에 한국어 본문을 발송
...
Infobip의 scope 카테고리에서 **SMS 관련**만 선택한다.
...
> **권장:** 운영 키는 `sms:send`만 부여. `numbers:manage`는 KISA 등록/변경 시점에만 분리된 bootstrap 키로 부여 후 즉시 폐기.
  ...
  "messages": [
    {
      ...
      "status": { "groupId": 1, "name": "MESSAGE_ACCEPTED" },
...
- 발송 성공 판정: `status.groupId === 1 (PENDING)` 또는 `3 (DELIVERED)`
- verify는 Grapit 백엔드(`SmsService.verifyCode`)에서 Valkey에 저장한 OTP와 사용자 입력을 비교. Infobip verify 엔드포인트는 **사용하지 않는다**.

> **`to` 형식 주의**: Grapit 코드는 `+`를 떼고 보낸다. `+821012345678` -> `821012345678`. cURL 테스트 시에도 같은 규칙을 따라야 한다.
...
결제 정보를 등록해도 **API Key는 그대로 유효**하다. `INFOBIP_SENDER`만 KISA 등록 완료된 운영 sender로 바꾸면 된다.

---

### 1.7 환경변수 정리

최종적으로 모노레포 루트 `.env`에 다음 **3개**를 채운다.
...
- 3종 중 하나라도 production에서 누락 -> SmsService 생성자 throw.
- Dev/test에서 3종 모두 비면 dev mock 모드(`000000` 통과) 활성.
- `App ` prefix 중복 경고(1.5.5)는 유지 -- raw key 값만 넣는다.
...
| Personal Base URL | 대시보드 상단 / API Docs 페이지 상단 | `INFOBIP_BASE_URL` | `https://xxxxx.api.infobip.com` (스킴 포함, 끝 슬래시 없음) | `${baseUrl}/sms/3/messages` URL 조립 |
| Sender ID | KISA 등록 번호 또는 Free Trial 기본값 | `INFOBIP_SENDER` | 숫자형 번호 (예: `0212345678`) 또는 비워두기 | `POST /sms/3/messages` body의 `from` 필드 |
...
node -e "
  ...
  console.log('SENDER:', sender || '(empty -- Free Trial default)');
"
```
...
SMS API v3는 요청 body의 `from` 필드에 sender를 직접 넣는다. Active 상태의 sender를 `INFOBIP_SENDER` 환경변수에 저장하면 된다 (1.3 참고).

**발송 검증 체크리스트**
- [ ] Sender 상태가 `Active` 인지 콘솔에서 확인
- [ ] `INFOBIP_SENDER` 환경변수에 Active sender 값이 설정되었는지
- [ ] API 응답의 `status.name` 이 `MESSAGE_ACCEPTED` 인지 (1.6 참고)
...
- [ ] **Infobip 2FA 리소스 3종 생성 완료** (§1.3~§1.5 참고)
  - Sender ID가 Infobip Portal에서 Active 상태
...
# === Infobip 3종 (전부 채워야 dev mock이 꺼짐) ===
...
- `INFOBIP_BASE_URL`은 계정마다 sub-domain이 다르다. 끝에 `/`나 `/sms/3/messages` 같은 경로 붙이지 않음.
...
이 줄이 떴다면 -> 3.3의 INFOBIP 3종 중 하나가 빠졌거나 오타. `.env` 다시 점검 후 재기동.
...
#### (b) `INFOBIP_BASE_URL` / `INFOBIP_SENDER` 오타 -- 4xx

**증상**: `Infobip API 400: Bad Request` 또는 `403: Sender not allowed`.

**해결**:
1. `INFOBIP_BASE_URL`에 `https://` 포함, 끝 슬래시 없는지 확인.
2. `INFOBIP_SENDER`가 Infobip Portal에서 Active 상태인지 확인 (한국은 KISA 등록 필수).
3. `.env` 값 trim -- 공백/개행 제거.
4. API 재기동 (`nest start --watch`는 `.env` 변경 자동 reload하지 않으므로 Ctrl+C 후 `pnpm dev`).
...
```
Error: [sms] INFOBIP_API_KEY, INFOBIP_BASE_URL, INFOBIP_SENDER required in production. Silent dev mock disabled.
```
...
- [ ] **Infobip 3종 값 손에 들고 있기** (1.7 참고)
  - `INFOBIP_API_KEY`, `INFOBIP_BASE_URL`, `INFOBIP_SENDER`
  - 블로커: API Key는 재표시 불가. 분실 시 1.5부터 재발급
- [ ] **Infobip Sender ID Active 상태 확인**
  - Infobip Portal > Senders에서 KISA 등록 번호가 Active인지 확인
...
#### 4.2.2 INFOBIP_* 시크릿 3종 생성

```bash
# 기존 2FA 전용 시크릿 제거 (v3 전환으로 불필요)
for SECRET in infobip-application-id infobip-message-id; do
  gcloud secrets delete "$SECRET" \
    --project=grapit-prod \
    --quiet 2>/dev/null || echo "skip: $SECRET (not found)"
done
...
# 3) Sender ID
echo -n "<INFOBIP_SENDER>" | gcloud secrets create infobip-sender \
  --project=grapit-prod \
  --replication-policy=user-managed \
  --locations=asia-northeast3 \
  --data-file=-
...
```bash
for SECRET in infobip-api-key infobip-base-url infobip-sender; do
...
`apps/api` 잡의 `deploy-cloudrun@v3` step `secrets:` 리스트에 3줄 추가:
...
gcloud run services update grapit-api \
  ...
  --update-secrets=INFOBIP_API_KEY=infobip-api-key:latest,INFOBIP_BASE_URL=infobip-base-url:latest,INFOBIP_SENDER=infobip-sender:latest \
  --remove-env-vars=TWILIO_ACCOUNT_SID,TWILIO_AUTH_TOKEN,TWILIO_VERIFY_SERVICE_SID
```

> **수동 update 직후 반드시 deploy.yml 도 동일하게 수정 + 커밋**. 안 그러면 다음 main 머지에서 INFOBIP_* 3종이 자동으로 사라진다.
...
> **중요**: `sms.service.ts` 의 production guard 가 INFOBIP_* 누락 시 생성자에서 throw -> NestJS 부팅 실패 -> `/health` 가 5xx가 아니라 **connection refused / 503**으로 돌아온다. 200 이 떨어지면 3종 시크릿이 정상 마운트된 것.
...
7. **재발송 후 재검증**: 새 OTP 발급 후 정상 검증 확인
