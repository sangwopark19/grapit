...
must_haves:
  truths:
    - "globals.css의 @theme 블록 안에 --shadow-sm/--shadow-md + --radius-sm/md/lg/xl + @keyframes seat-checkmark-fade-in 정의가 추가되어 Tailwind v4가 shadow-sm/rounded-lg 등 utility를 자동 생성한다"
    - "globals.css 외부 (@theme 블록 종료 후)에 [data-seat-checkmark] selector + prefers-reduced-motion override가 추가되어 Plan 12-03의 viewer 변경 시 체크마크 fade-in이 자동 작동 + reduced-motion 시 즉시 표시"
  ...
  artifacts:
    - path: "apps/web/app/globals.css"
      provides: "신규 shadow/radius 토큰 + seat-checkmark-fade-in keyframe + reduced-motion override"
      contains: "--shadow-sm, --shadow-md, --radius-sm, --radius-md, --radius-lg, --radius-xl, @keyframes seat-checkmark-fade-in, [data-seat-checkmark], prefers-reduced-motion"
  ...
  key_links:
    ...
    - from: "apps/web/app/globals.css [data-seat-checkmark] selector"
      to: "Plan 12-03 seat-map-viewer.tsx 체크마크 <text>"
...
D-01/D-02 (Minimalism, shadow sm+md만, radius 6~12px 범위) + D-04 (홈 파일럿, 3~5줄) + D-12 (체크마크 fade-in을 위한 CSS @keyframes 인프라)를 단일 plan으로 묶는다. 두 변경 모두 후속 wave의 seat-map-viewer / svg-preview / use-is-mobile에 dependency 없음 — Wave 1 단독 진행 가능.

Purpose:
- shadcn 컴포넌트(Button/Card/Tooltip 등)가 JSX 변경 0줄로 새 elevation/radius 톤을 자동 흡수 (UX-01)
- Plan 12-03이 viewer 체크마크에 `data-seat-checkmark` 속성만 추가하면 fade-in 애니메이션이 자동 작동하도록 CSS 인프라 선행 구축 (UX-04 보조)
- 홈 페이지(`/`)에서 새 디자인 톤을 시각적으로 확인할 수 있도록 섹션 간격 1줄씩 조정 (UX-01 파일럿)

Output:
- `apps/web/app/globals.css` — 신규 토큰 6개 + keyframe 1개 + 외부 selector 2개
...
- `@media (prefers-reduced-motion: reduce)` 안에서 `[data-seat-checkmark]`는 `animation: none; opacity: 1;` — 즉시 표시.
...
<task type="auto">
  <name>Task 1: globals.css에 shadow/radius 토큰 + seat-checkmark fade-in 인프라 추가</name>
...
추가로, @theme 블록 안 기존 `@keyframes shake` (line 95~100) 직후, @theme 블록 닫는 `}` (line 101) 직전에 다음 keyframe을 INSERT:

```css

  /* Phase 12 (D-12): Seat selection checkmark fade-in keyframe */
...
변경 후 (`:root` 블록 닫는 `}` 직후, `html {` 직전에 다음 블록을 INSERT):

```css

/* Phase 12 (D-12): Seat checkmark animation — auto-triggered on mount.
 * Plan 12-03 viewer adds `data-seat-checkmark` attr to selected seat <text>;
 * this CSS fades it in over 150ms. prefers-reduced-motion 시 즉시 표시.
 */
[data-seat-checkmark] {
...
- `@keyframes seat-checkmark-fade-in` 도 @theme 블록 안에 두는 것이 기존 패턴(`@keyframes enter/exit/shake` 모두 @theme 안). Tailwind v4가 인식.
- `[data-seat-checkmark]` selector + `prefers-reduced-motion` 미디어 쿼리는 namespace 외 일반 CSS — `@theme` **밖** 정의. (PATTERNS.md §"reduced-motion override 패턴")
...
- 색상 토큰, 스페이싱 토큰, 타이포 토큰은 변경 없음 — D-03 (브랜드 Purple 유지) 보장.
  ...
  <verify>
    <automated>cd /Users/sangwopark19/icons/grapit && grep -E "^\s*--shadow-sm: 0 1px 2px 0 rgba\(0, 0, 0, 0.05\)" apps/web/app/globals.css && grep -E "^\s*--shadow-md: 0 4px 12px -2px rgba\(0, 0, 0, 0.08\)" apps/web/app/globals.css && grep -E "^\s*--radius-sm: 6px" apps/web/app/globals.css && grep -E "^\s*--radius-md: 8px" apps/web/app/globals.css && grep -E "^\s*--radius-lg: 10px" apps/web/app/globals.css && grep -E "^\s*--radius-xl: 12px" apps/web/app/globals.css && grep -E "@keyframes seat-checkmark-fade-in" apps/web/app/globals.css && grep -E "\\[data-seat-checkmark\\]" apps/web/app/globals.css && grep -E "prefers-reduced-motion: reduce" apps/web/app/globals.css && pnpm --filter @grapit/web typecheck 2>&1 | tail -5</automated>
  </verify>
  <acceptance_criteria>
    ...
    - keyframe이 `@theme` 블록 안에 있는지 위치 검증 (라인 번호 비교):
      - keyframe seat-checkmark-fade-in의 line ≤ closing `@theme` brace의 line. 정확히는 `awk` 또는 `grep -n` 결과 비교 — 실패 시 `[data-seat-checkmark]`가 @theme 밖에 있는지도 함께 확인. 시각 검증으로 충분.
    - 외부 selector + 미디어 쿼리 존재:
      - `grep -q "^\[data-seat-checkmark\]" apps/web/app/globals.css`
  ...
  <done>
globals.css에 shadow/radius 6 토큰 + seat-checkmark-fade-in keyframe + [data-seat-checkmark] selector + prefers-reduced-motion override 정의됨. 기존 컬러/스페이싱/타이포/chart 토큰 회귀 0. Tailwind v4가 새 utility 자동 생성. Plan 12-03이 viewer 체크마크에 `data-seat-checkmark` attr만 추가하면 fade-in 자동 작동 가능.
  </done>
</task>
...
<verification>
- [ ] globals.css `@theme` 블록 안 6개 토큰(--shadow-sm/md, --radius-sm/md/lg/xl) + 1개 keyframe(seat-checkmark-fade-in) 정의 확인
- [ ] globals.css `@theme` 블록 밖 [data-seat-checkmark] selector + prefers-reduced-motion override 정의 확인
...
- 수동(Wave 4 manual QA gate에서 검증): dev server `/` 진입 시 홈 섹션 간 수직 리듬이 toned (mt-10) 시각 확인. dev server `/admin/dashboard` 카드 elevation/radius가 modernize 시각 확인 (UX-01).
</success_criteria>

<output>
After completion, create `.planning/phases/12-ux/12-01-SUMMARY.md`:
- globals.css 추가된 토큰/keyframe/selector 라인 인용
- 홈 3개 파일 변경 라인 인용 (1줄 × 3)
- typecheck/lint 결과
- Wave 4 manual QA에서 검증할 시각 항목 리스트
</output>
