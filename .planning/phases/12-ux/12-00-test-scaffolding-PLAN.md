...
must_haves:
  truths:
    ...
    - "B-2-RESIDUAL: seat-map-viewer.test.tsx 신규 케이스 1이 *useEffect 기반 fill 변경*을 검증 — vitest renderer가 마운트 후 useEffect 실행 → `seatA1.style.transition`에 `fill 150ms` 포함 + `seatA1.getAttribute('fill')`이 primary로 변경됨 검증 (Option C)"
    - "W-2: prefix-svg-defs-ids.test.ts 5 케이스(<defs> 없음 / ID 1개 / 다수 / 정규식 메타문자 escape / parse 실패 graceful)가 helper 단위 테스트로 작성됨"
  artifacts:
    ...
    - path: "apps/web/components/booking/__tests__/seat-map-viewer.test.tsx"
      provides: "UX-04/05/06 회귀 + 신규 6 케이스 (transition useEffect 기반 / data-seat-checkmark / MiniMap / initialScale / STAGE 오버레이 / B-2-RESIDUAL data-fading-out + DOM 잔존)"
      contains: "vi.hoisted (B-3) + MiniMap mock + transformWrapperSpy + useIsMobile mock + Option C useEffect 검증 (style.transition + fill primary)"
    - path: "apps/web/components/booking/__utils__/__tests__/prefix-svg-defs-ids.test.ts"
      provides: "W-2: prefixSvgDefsIds helper 5 케이스 (no defs / 1 id / multiple / regex meta / parse error)"
      contains: "describe('prefixSvgDefsIds') + 5 케이스, 직접 named import"
  key_links:
...
**Revision (B-3/B-4/B-2-RESIDUAL/W-2 적용):**
...
- B-2-RESIDUAL Option C: seat-map-viewer.test.tsx 신규 케이스 1을 *useEffect 기반 fill 변경* 검증으로 갱신 (`seatA1.style.transition`에 'fill 150ms' 포함 + `seatA1.getAttribute('fill')`이 primary로 변경됨)
- B-2-RESIDUAL: 신규 케이스 7도 *체크마크 data-fading-out + DOM 잔존 → 160ms 후 제거*만 검증 (data-fading-in은 더 이상 사용 안 함 — useEffect가 fill 직접 변경)
...
- 갱신 `apps/web/components/booking/__tests__/seat-map-viewer.test.tsx` (B-3 vi.hoisted 적용 mock 확장 + 6 신규 케이스 — B-2-RESIDUAL Option C useEffect 기반 검증)
- 신규 `apps/web/components/booking/__utils__/__tests__/prefix-svg-defs-ids.test.ts` (W-2: 5 케이스)
</objective>
...
- 변경 1 (UX-04 + B-2-RESIDUAL Option C): useMemo 안 selected 분기에서 inline `transition:fill 150ms` 부여를 *제거*. 별도 useEffect가 마운트 직후 동일 element에 `el.style.transition = 'fill 150ms ease-out'` 부여 + `el.setAttribute('fill', '#6C3CE0')` 변경 → 동일 element 속성 변경이므로 CSS transition 정상 발화.
- 변경 2 (UX-04): 선택 좌석 체크마크 `<text>`에 `setAttribute('data-seat-checkmark', '')` 한 줄 추가 (mount 시 fade-in — CSS @keyframes mount-time 트리거)
- 변경 2b (B-2-RESIDUAL Option C): `pendingRemovals` (useState<Set<string>>) 메커니즘 도입
  - `[prevSelected - currentSelected]` 차집합 → 해제된 좌석 → 체크마크 `<text>`를 즉시 제거하지 않고 `data-fading-out="true"` 부여 + 다음 useMemo 사이클에서 *체크마크 마운트 유지* + 150ms 후 pendingRemovals reset
  - 해제 분기에서 fill을 *원래 tier 색*으로 복원하는 것은 useEffect (deps에 pendingRemovals)
  ...
  - **pendingSelections는 사용하지 않음** (Option C: useEffect가 fill을 즉시 변경하면 transition 자동 발화 — 별도 pending state 불필요)
...
<task type="auto" tdd="true">
  <name>Task 3: seat-map-viewer 테스트 mock 확장 (B-3 vi.hoisted) + 6 신규 케이스 추가 (B-2-RESIDUAL Option C: useEffect 기반 fill 검증)</name>
  ...
  <behavior>
    ...
    - 신규 6 케이스 추가:
      1. **B-2-RESIDUAL Option C**: 선택 좌석에 useEffect가 적용한 `el.style.transition`에 `fill 150ms` 포함 + `getAttribute('fill')`이 primary `#6C3CE0`으로 변경됨 검증 (vitest renderer가 마운트 후 useEffect 실행)
      ...
      7. **B-2-RESIDUAL Option C**: 해제 시 체크마크에 `data-fading-out="true"` 속성이 잠시 부여되고 160ms 후 DOM에서 제거됨 (pendingRemovals + setTimeout 메커니즘) — `vi.useFakeTimers()` + `vi.advanceTimersByTime(160)`로 검증
  </behavior>
  <action>
...
```tsx
  ...
  // 신규 케이스 1 (B-2-RESIDUAL Option C): useEffect가 마운트 후 fill을 primary로 변경 + el.style.transition에 fill 150ms 포함
  it('B-2-RESIDUAL Option C: 선택 좌석에 useEffect가 el.style.transition=fill 150ms 부여 + getAttribute("fill")이 primary로 변경 (UX-04)', async () => {
...
`;
  ...
  // 신규 케이스 7 (B-2-RESIDUAL Option C): 해제 시 체크마크에 data-fading-out="true" 부여 + 160ms 후 DOM 제거
  it('B-2-RESIDUAL: 해제 시 체크마크에 data-fading-out="true" 부여되고 160ms 후 DOM에서 제거됨 (pendingRemovals)', async () => {
    ...
    try {
      ...
      // Phase 2: 해제 → data-fading-out 부여 + 체크마크 즉시 제거 안 됨
      rerender(
        <SeatMapViewer
          svgUrl="https://example.com/seats.svg"
          seatConfig={mockSeatConfig}
...
주의:
- 본 task 완료 후 vitest 실행 시 신규 6 케이스(transition useEffect / data-seat-checkmark / MiniMap 마운트 / initialScale / STAGE 오버레이 / B-2-RESIDUAL data-fading-out)는 RED. 12-03 viewer 변경 완료 후 GREEN으로 전환.
...
- **B-2-RESIDUAL Option C**: 신규 케이스 1은 *useEffect 기반* fill 변경 검증 — `seatA1.style.transition`에 'fill 150ms' 포함 + `seatA1.getAttribute('fill')`이 primary `#6C3CE0`으로 변경됨. waitFor로 useEffect 실행 시점 polling. 12-03 Task 3이 useEffect 추가하면 GREEN.
- **B-2-RESIDUAL**: 신규 케이스 7은 *체크마크 data-fading-out + DOM 잔존*만 검증 (data-fading-in 검증 제거 — Option C에서는 pendingSelections 미사용).
- **B-2-RESIDUAL fakeTimers**: `vi.useFakeTimers({ shouldAdvanceTime: true })` 옵션은 microtask가 timer queue 외부에서 진행되도록 허용 — render/rerender의 setTimeout이 자연스럽게 동작.
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - 신규 케이스 grep (각 케이스의 식별 문자열):
      - **B-2-RESIDUAL Option C**: `grep -q "seatA1.style.transition" /Users/sangwopark19/icons/grapit/apps/web/components/booking/__tests__/seat-map-viewer.test.tsx`
      - **B-2-RESIDUAL Option C**: `grep -q "fill 150ms" /Users/sangwopark19/icons/grapit/apps/web/components/booking/__tests__/seat-map-viewer.test.tsx`
      - **B-2-RESIDUAL Option C**: `grep -q "6c3ce0\\|6C3CE0" /Users/sangwopark19/icons/grapit/apps/web/components/booking/__tests__/seat-map-viewer.test.tsx`
      ...
      - **B-2-RESIDUAL**: `grep -q "data-fading-out" /Users/sangwopark19/icons/grapit/apps/web/components/booking/__tests__/seat-map-viewer.test.tsx`
      - **B-2-RESIDUAL**: `grep -q "advanceTimersByTime(160)" /Users/sangwopark19/icons/grapit/apps/web/components/booking/__tests__/seat-map-viewer.test.tsx`
    - 케이스 카운트 ≥ 12 (기존 6 + 신규 6 — STAGE/MiniMap/B-2-RESIDUAL 케이스 포함):
  ...
  <done>
seat-map-viewer.test.tsx가 mock 확장 (B-3 vi.hoisted 적용 + MiniMap + useIsMobile) + 신규 6 케이스 추가 완료 (B-2-RESIDUAL Option C: useEffect 기반 fill 검증 + data-fading-out DOM 잔존 검증). 기존 6 회귀 케이스는 GREEN 유지. ReferenceError 0건 (B-3). 12-03 구현 후 신규 케이스도 GREEN 전환 가능한 상태.
  </done>
</task>

<task type="auto" tdd="true">
  <name>Task 4 (W-2): prefixSvgDefsIds helper 단위 테스트 5건 신규 작성</name>
...
import { prefixSvgDefsIds } from '../prefix-svg-defs-ids';

describe('prefixSvgDefsIds (W-2: SVG <defs> ID 충돌 방지)', () => {
  it('<defs> 없는 SVG는 원본 그대로 반환', () => {
    const svg =
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 200"><rect data-seat-id="A-1" x="10" y="10" width="32" height="32"/></svg>';
  ...
  it('<defs> 안 ID 1개 → ID에 prefix 부여 + url(#) 일괄 치환', () => {
    const svg =
  ...
  it('ID에 정규식 메타문자가 포함되어도 escape되어 정상 치환 (gradient.1)', () => {
    const svg =
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 200">' +
  ...
  it('parse 실패 (잘못된 XML)도 graceful — 원본 반환', () => {
    // <defs를 포함하지만 XML로는 invalid → DOMParser parsererror 또는 try-catch 진입
...
- Wave 0 종료 시 import 자체가 실패 → 모든 케이스 RED. 12-03 Task 1 완료 후 GREEN.
- jsdom DOMParser는 invalid XML 시 `documentElement.tagName === 'parsererror'`를 반환 — 헬퍼는 이 가드 + try-catch로 graceful 처리.
- 정규식 메타문자 escape (`.`, `*`, `+`, `?`, `^`, `$`, `{}`, `()`, `|`, `[]`, `\`) — 헬퍼가 `oldId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')` 사용.
  </action>
  <verify>
    <automated>cd /Users/sangwopark19/icons/grapit && pnpm --filter @grapit/web test -- prefix-svg-defs-ids --run 2>&1 | tail -30</automated>
  </verify>
  <acceptance_criteria>
    ...
    - import 검증:
      - `grep -q "import { prefixSvgDefsIds } from '../prefix-svg-defs-ids'" /Users/sangwopark19/icons/grapit/apps/web/components/booking/__utils__/__tests__/prefix-svg-defs-ids.test.ts`
    - 5개 케이스 식별 문자열 grep:
    ...
    - vitest 실행 (구현 전): exit code 1 (모듈 import 실패) 허용. Wave 0 게이트는 "케이스가 카운트되어 실행 시도됨"으로 충분.
    - typecheck 회귀 없음: `pnpm --filter @grapit/web typecheck` exit 0
  </acceptance_criteria>
  <done>
prefix-svg-defs-ids.test.ts가 5개 케이스로 작성되어 12-03 Task 1의 helper 구현 검증 진입점이 마련됨. W-2 단위 테스트 매핑 확립.
  </done>
</task>
...
<verification>
- [ ] 신규 파일 3개(`svg-preview.test.tsx`, `use-is-mobile.test.ts` — B-4 케이스 포함, `prefix-svg-defs-ids.test.ts` — W-2 5 케이스) 생성됨
- [ ] 기존 파일 1개(`seat-map-viewer.test.tsx`) B-3 vi.hoisted 적용 mock 확장 + 6 케이스 추가됨 (B-2-RESIDUAL Option C: useEffect 기반 fill 검증 + data-fading-out DOM 잔존 검증)
...
- B-3/B-4/B-2-RESIDUAL/W-2 가드 확립: ReferenceError 0건 + getServerSnapshot SSR fallback unit test + Option C useEffect 기반 fill 검증 + data-fading-out DOM 잔존 검증 + helper 5 케이스
</success_criteria>

<output>
After completion, create `.planning/phases/12-ux/12-00-SUMMARY.md`:
- 신규/갱신 파일 4개 + 케이스 카운트 (svg-preview 4, use-is-mobile 4, seat-map-viewer 12, prefix-svg-defs-ids 5)
...
- B-2-RESIDUAL Option C: useEffect 기반 fill 검증 케이스 추가 증거 + data-fading-out DOM 잔존 검증 케이스 추가 증거
