...
### 5. 토스 결제 위젯 렌더링
expected: 확인 페이지에서 토스 결제 위젯이 인라인으로 렌더링되며, 카드/카카오페이/네이버페이/계좌이체 등 결제 수단이 표시됩니다. (NEXT_PUBLIC_TOSS_CLIENT_KEY 환경변수 필요. 없으면 에러 메시지가 graceful하게 표시됨)
result: blocked
blocked_by: third-party
reason: "토스 키 미발급. graceful 에러 메시지 정상 표시됨."

### 6. 결제 완료 페이지
expected: 결제 성공 후 /booking/[performanceId]/complete로 리다이렉트됩니다. 예약 번호, 공연 정보, 좌석 정보, 결제 금액이 표시되며, 메인/마이페이지 이동 CTA 버튼이 있습니다.
result: blocked
blocked_by: third-party
reason: "토스 키 미발급으로 결제 테스트 불가"

### 7. 완료 페이지 새로고침 복구
expected: 결제 완료 페이지에서 브라우저 새로고침(F5)해도 orderId 쿼리 파라미터를 통해 예약 정보가 정상 로드되어 동일한 화면을 유지합니다.
result: blocked
blocked_by: third-party
reason: "토스 키 미발급으로 결제 완료 페이지 접근 불가"
...
result: blocked
blocked_by: third-party
reason: "테스트 데이터 없음 — 토스 결제 연동 후 재테스트 필요"
